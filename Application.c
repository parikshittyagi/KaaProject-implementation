/*In this Code Schema is taken to be humidity as a parameter and
  its random value is being generated */
...
static int32_t get_humidity_sample(void)
{
...
static void example_callback(void *context)
{
    ...
    if (difftime(current_time, last_sample_time) >= sample_period) {
        int32_t humidity = get_humidity_sample();
        printf("Sampled humidity: %i\n", humidity);
...
int main(void)
{
    /* Init random generator used to generate humidity */
    srand(time(NULL));
    /* Prepare Kaa client. */
